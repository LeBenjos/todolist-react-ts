...
export default function HeaderRight({
  ...
  const [addingMember, setAddingMember] = useState<boolean>(
    window.location.href.split("/")[5] === LinkPath.PROJECT_ADD_MEMBER
      ? true
      : false
  );
    ...
    <div className="header-right">
          ...
          {project.participants
            ...
            .map((participant, index) => {
              return (
                <div key={index}>
                  <img
                    src={ImagePath.DEFAULT_PICTURE_RED}
      ...
      <div>
        <img
          src={
            userData
              ? ImagePath.DEFAULT_PICTURE_GREEN
              : ImagePath.DEFAULT_PICTURE_RED
          }
          alt="Default Profil Picture"
        />
      ...
      {id &&
        ...
        (addingMember ? (
          ...
          >
            {HeaderText.BUTTON_BACK_PROJECT}
          </Link>
        ) : (
          <Link
            className="buttonCta"
            to={`project/${id}/add_member`}
            onClick={() => setAddingMember(!addingMember)}
          >
            {HeaderText.BUTTON_ADD_MEMBER}
          </Link>
        ))}
    </div>
  );
